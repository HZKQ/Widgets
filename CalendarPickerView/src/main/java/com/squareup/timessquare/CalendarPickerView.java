...
public class CalendarPickerView extends ListView {
    ...
    private FluentInitializer mFluentInitializer;
    ...
    public FluentInitializer init(Date minDate, Date maxDate, TimeZone timeZone, Locale locale) {
        ...
        validateAndUpdate();
        mFluentInitializer = new FluentInitializer();
        return mFluentInitializer;
    }
    ...
    public class FluentInitializer {
        ...
        /**
         * Set multiple selected dates.  This will throw an IllegalArgumentException if you
         * pass in multiple dates and haven't already called {@link #inMode(SelectionMode)}.
         */
        public FluentInitializer withSelectedDates(Date... selectedDates) {
    ...
    /**
     * 选择区间，当且仅当{@link #selectionMode}为{@link SelectionMode#RANGE}时有效
     */
    public void selectRangeDate(Date startTime, Date endTime) {
        if (selectionMode != SelectionMode.RANGE) {
            return;
        }
        
        mFluentInitializer.withSelectedDates(startTime, endTime);
    }
    
    /**
     * 选择多个但不连续的时间点，当且仅当{@link #selectionMode}为{@link SelectionMode#MULTIPLE}时有效
     */
    public void selectMultipleDate(Date... dates) {
        selectMultipleDate(Arrays.asList(dates));
    }
    
    /**
     * 选择多个但不连续的时间点，当且仅当{@link #selectionMode}为{@link SelectionMode#MULTIPLE}时有效
     */
    public void selectMultipleDate(Collection<Date> dates) {
        if (selectionMode != SelectionMode.MULTIPLE) {
            return;
        }
        
        mFluentInitializer.withSelectedDates(dates);
    }
