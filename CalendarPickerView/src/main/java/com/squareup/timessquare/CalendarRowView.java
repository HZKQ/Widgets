...
public class CalendarRowView extends ViewGroup implements View.OnClickListener {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int c = 0, numChildren = getChildCount(); c < numChildren; c++) {
            ...
            int l = ((c + 0) * totalWidth) / 12;
            int r = ((c + 1) * totalWidth) / 12;
            int cellSize = r - l;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int c = 0, numChildren = getChildCount(); c < numChildren; c++) {
            final View child = getChildAt(c);
            int l = ((c + 0) * width) / 7 + cellHeight / 3;
            int r = ((c + 1) * width) / 7 + cellHeight / 3;
    ...
    public void setCellBackground(int resId) {
        for (int i = 0; i < getChildCount(); i++) {
            TextView tv = ((CalendarCellView) getChildAt(i)).getDayOfMonthTextView();
            if (tv != null) {
