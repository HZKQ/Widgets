...
public class ShadowLayout extends FrameLayout {
    ...
    private final RectF mRectFStroke = new RectF();
        ...
        if (selectorType == 1 && firstView != null) {
            ...
            if (!isClickable) {
                ...
                if (clickAbleFalseColor != -101) {
                    ...
                    // 说明设置了背景图
                    setBackGround(clickAbleFalseDrawable, "changeSwitchClickable");
                ...
                // 可点击的状态
                if (layoutBackground != null) {
                    setBackGround(layoutBackground, "changeSwitchClickable");
                } else {
            ...
            if (selectorType == 2) {
                if (selected) {
                    ...
                    if (layoutBackground_true != null) {
                        setBackGround(layoutBackground_true, "setSelected");
                    }
                    ...
                    if (layoutBackground != null) {
                        setBackGround(layoutBackground, "setSelected");
                    }
    ...
    /**
     * 设置圆角半径，如果设置了此值，也调用{@link #setSpecialCorner(int, int, int, int)}分别指定每个角的圆角半径，
     * 则取两者中较大值
     */
    ...
    /**
     * 分别设置四个角圆角半径，如果设置了此值，也调用{@link #setCornerRadius(int)}指定整体圆角半径，则取两者中较大值
     */
    ...
    protected void onFinishInflate() {
        ...
        if (selectorType == 2) {
            setBackGround(layoutBackground, "onFinishInflate");
        } else {
            if (isClickable) {
                setBackGround(layoutBackground, "onFinishInflate");
            } else {
                setBackGround(clickAbleFalseDrawable, "onFinishInflate");
                if (clickAbleFalseColor != -101) {
                    paint.setColor(clickAbleFalseColor);
                }
            }
        ...
        if (isShowShadow) {
            ...
            if (getChildAt(0) == null) {
                if (layoutBackground != null) {
                    firstView = ShadowLayout.this;
                    if (isClickable) {
                        setBackGround(layoutBackground, "setBackgroundCompat");
                    } else {
                        changeSwitchClickable();
                    }
                } else {
    ...
    private void initAttributes(AttributeSet attrs) {
        ...
        try {
            ...
            stroke_with = attr.getDimension(R.styleable.ShadowLayout_hl_strokeWith, getResources().getDimension(R.dimen.dp_1));
        ...
        if (!isInEditMode()) {
            mCornerRadius = SimpleUtil.getScaledValue(mCornerRadius);
            mCornerRadius_leftTop = SimpleUtil.getScaledValue(mCornerRadius_leftTop);
            mCornerRadius_leftBottom = SimpleUtil.getScaledValue(mCornerRadius_leftBottom);
            mCornerRadius_rightTop = SimpleUtil.getScaledValue(mCornerRadius_rightTop);
            mCornerRadius_rightBottom = SimpleUtil.getScaledValue(mCornerRadius_rightBottom);
            mShadowLimit = SimpleUtil.getScaledValue(mShadowLimit);
            mDx = SimpleUtil.getScaledValue(mDx);
            mDy = SimpleUtil.getScaledValue(mDy);
            stroke_with = SimpleUtil.getScaledValue(stroke_with);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        rectf.bottom = getHeight() - bottomPadding;
        int trueHeight = (int) (rectf.bottom - rectf.top);
        // 如果都为-1说明，没有设置特定角，那么按正常绘制
        if (mCornerRadius_leftTop == -1 && mCornerRadius_leftBottom == -1 && mCornerRadius_rightTop == -1 && mCornerRadius_rightBottom == -1) {
            if (mCornerRadius > trueHeight / 2f) {
                if (selectorType != 3) {
                    if (layoutBackground == null && layoutBackground_true == null) {
                        // 画圆角矩形
                        canvas.drawRoundRect(rectf, trueHeight / 2f, trueHeight / 2f, paint);
                        // 解决边框线太洗时，四角的width偏大和其他边不同为什大姨夫啊被烦死了
                        if (stroke_color != -101) {
                            mRectFStroke.set(rectf.left + stroke_with / 2f,
                                rectf.top + stroke_with / 2f,
                                rectf.right - stroke_with / 2f,
                                rectf.bottom - stroke_with / 2f);
                            
                            canvas.drawRoundRect(mRectFStroke,
                                trueHeight / 2f - stroke_with / 2f,
                                trueHeight / 2f - stroke_with / 2,
                                paint_stroke);
                        }
                    }
    ...
    private float[] getCornerValue(int trueHeight) {
        return getCornerValueOther(trueHeight, 0);
    }
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        if (selectorType == 1) {
                            ...
                            if (layoutBackground_true != null) {
                                setBackGround(layoutBackground_true, "onTouchEvent");
                            }
                            postInvalidate();
                    ...
                    case MotionEvent.ACTION_UP:
                        if (selectorType == 1) {
                            ...
                            if (layoutBackground != null) {
                                setBackGround(layoutBackground, "onTouchEvent");
                            }
                            postInvalidate();
    ...
    private void setBackGround(Drawable drawable, String currentTag) {
