 ...
 * 构建富文本
 *
 * @author Created by wanggaowan on 2021/8/13 20:45
 */
public class SpannableString extends android.text.SpannableString {
    ...
    public static class AppendBuilder {
        ...
        public <T extends TextView> void apply(T textView) {
            ...
            for (AppendSpan span : mSpans) {
                ...
                ss.setSpan(new TextSpan(span), start, end, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                if (span.couldClick && span.clickListener != null) {
                    needClick = true;
                    ss.setSpan(new ClickSpan(span), start, end, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                }
                start = end;
            }
            ...
            if (needClick) {
                if (!textView.isTextSelectable()) {
                    // 如果textView设置了ClickableSpan，但是不可选中时，需要设置长按事件且始终返回true，否则抛出
                    // android.widget.Editor$SelectionModifierCursorController.getMinTouchOffset() on a null object 异常
    ...
    public static class AppendSpan {
        ...
        protected float size = INVALID_VALUE;
         ...
         */
        public AppendSpan size(float size) {
            this.size = size;
            return this;
        }
    ...
    public static class Builder extends AppendBuilder {
        ...
        public <T extends TextView> void apply(T textView) {
            ...
            for (Span span : mSpans) {
                ss.setSpan(new TextSpan(span), span.start, span.end, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                if (span.couldClick && span.clickListener != null) {
                    needClick = true;
                    ss.setSpan(new ClickSpan(span), span.start, span.end, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                }
            }
    ...
    /**
     * 用于实现{@link AppendSpan}中设置的内容,点击除外
     */
    static class TextSpan extends MetricAffectingSpan {
        
        private final AppendSpan span;
        
        TextSpan(AppendSpan span) {
            this.span = span;
        }
        ...
        /**
         * 获取文本字体大小，如果未设置，则返回{@link AppendSpan#INVALID_VALUE}
         */
        ...
        public void updateMeasureState(@NonNull TextPaint textPaint) {
            if (span.size != AppendSpan.INVALID_VALUE) {
                textPaint.setTextSize(span.size);
            }
        }
    }
    
    /**
     * 用于实现{@link AppendSpan}中点击事件
     */
    static class ClickSpan extends ClickableSpan {
        
        private final AppendSpan span;
        
        ClickSpan(AppendSpan span) {
            this.span = span;
        }
